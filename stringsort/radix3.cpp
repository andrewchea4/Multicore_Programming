...
void prefix_sum(int idx, int left, bool f_flag, int* pair_sum, int* prefix_array)
{
	if(f_flag){
		...
		//	if(2*idx >= 64*16 && 2*idx <= 64*16 + 17){
                                //cout << "lf_input :" << lf_input << " " << "total lf_input :" << lf_input + pair_sum[2*idx] << " " << "rt_input :" << rt_input << " " << "total rt_input : "<< rt_input + pair_sum[2*idx+1] << endl;
                                //cout << "left array :" << prefix_array[2*idx - 64*16] << " " << "right array :" << prefix_array[2*idx + 1 - 64*16] << endl;
                       // }
			#pragma omp taskwait
			return;
		}
...
#pragma omp parallel num_threads(16) shared(hist_res) shared(count_arr) firstprivate(hist) firstprivate(flag) firstprivate(count) firstprivate(index)
{
	...
	#pragma omp critical
	{
		//cout << "Thread ID :" << omp_get_thread_num() << " " << "Count : " << count << endl;
		for(int j = 0; j<64; j++)
			hist_res[index/(N/omp_get_num_threads())][j] = hist[j];
		count_arr[index/(N/omp_get_num_threads())][0] = index;
		count_arr[index/(N/omp_get_num_threads())][1] = count;

		cout << "INDEX :" << index/(N/omp_get_num_threads()) << " " << count_arr[index/(N/omp_get_num_threads())][0] << " " << count_arr[index/(N/omp_get_num_threads())][1] << endl;
	}		
#pragma omp barrier
}
...
//	cout << pair_sum[64*16] << " " << pair_sum[64*16 + 15] << " " << pair_sum[64*16 + 16] << endl;	
...
#pragma omp barrier
	...
	auto copy_str = new char[N][30];
#pragma omp parallel num_threads(16) shared(strArr) shared(copy_str)
{
	#pragma omp for
	for(int j = 0; j<N; j++){
		strcpy(copy_str[j], strArr[j]);
	}
#pragma omp barrier
}
...
#pragma omp parallel num_threads(16) shared(strArr) shared(index_arr) shared(count_arr) shared(count)
{
	...
	for(int j =start; j<start+end; j++){
		...
		{
			strcpy(copy_str[index_arr[start/(N/omp_get_num_threads())][0]], strArr[j]);
			index_arr[start/(N/omp_get_num_threads())][0]++;
		}
		else{
			strcpy(copy_str[index_arr[start/(N/omp_get_num_threads())][strArr[j][max_length-1] - 64]], strArr[j]);
			index_arr[start/(N/omp_get_num_threads())][strArr[j][max_length-1] - 64]++;
		//	cout << "Number :" << strArr[j][max_length-1] - 64 << " " << "Real number : " << index_arr[start/(N/omp_get_num_threads())][strArr[j][max_length-1] - 64] << endl;
		//	cout << "Original :" << strArr[j] << " " << "Sorted : " << copy_str[index_arr[start/(N/omp_get_num_threads())][strArr[j][max_length-1] - 64]] << endl;
		}
	}

}
	for(int j = 0; j<N; j++)
		cout << "Original :" << strArr[j] << " " << "Sorted : " << copy_str[j] << endl;
