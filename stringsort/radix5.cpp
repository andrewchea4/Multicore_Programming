...
int pairwise_sum(int i, int N, size_t max_length, char** strArr, char** copy_str)
{
...
#pragma omp parallel num_threads(16) shared(strArr) shared(index_arr) shared(count_arr) shared(count)
{
	...
	for(int j = start; j<start+end; j++){
		...
		{
			strcpy(copy_str[index_arr[thd][0]], strArr[j]);
			index_arr[thd][0]++;
		}
		else{
			strcpy(copy_str[index_arr[thd][strArr[j][max_length - 1]-64]], strArr[j]);
...
int main(int argc, char* argv[])
{
	...
	//int ** hist_res;
	//hist_res = (int **)malloc(sizeof(int *) *16);
	//for(int j = 0; j<16; j++)
	//	hist_res[j] = (int *)malloc(sizeof(int) * 64);
	
//	int *pair_sum;
//	pair_sum = (int *)malloc(sizeof(int) * 2048);
	
//	int *prefix_array;
//	prefix_array = (int *)malloc(sizeof(int) *1024);

	//auto hist_res = new int[16][64];
	//int **count_arr;
	//count_arr = (int **)malloc(sizeof(int *) *16);
	//for(int j = 0; j<16; j++)
	//	count_arr[j] = (int *)malloc(sizeof(int) * 64);
...
//	max_length = 1;
	max_length = pairwise_sum(i, N, max_length, strArr, copy_str);
	while(max_length != 0)
	{
		max_length = pairwise_sum(i, N, max_length, copy_str, copy_str);
	}
#pragma omp parallel num_threads(16) shared(strArr) shared(copy_str)
{
	#pragma omp for
	for(int j = 0; j<N; j++){
		strcpy(strArr[j], copy_str[j]);
	}
#pragma omp barrier
}	
	/*for(int j = 0; j<N; j++)
